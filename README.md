...
找到刚刚下载的 Shadowrocket 证书描述文件，然后点击安装，按照提示输入锁屏密码并完成安装。
4. 依次进入设置 → 通用 → 关于本机 → 证书信任设置
找到Shadowrocket生成的证书，打开对应的信任开关。
系统弹出警告后，确认启用完全信任。
5. 打开并添加两个快捷指令，分别是更改定位和恢复定位，快捷指令链接在项目中
这样就配置好了，具体更改定位的步骤看下面👇
6. 启用 shadowrocket 节点
7. 打开苹果地图，点开修改到的地点，点击“共享”，在共享菜单中选择“wloc 设置地理位置”，等待快捷指令运行完成。
8. 随后会自动跳转到定位页面，关闭定位后等待约 10 秒重新开启，再次打开地图，就会发现定位已经更改。
如果没有立即改变，可以尝试：关闭地图App，在后台任务列表中彻底划掉地图，关闭再打开系统定位服务，重新打开地图。或者重启手机
恢复定位的步骤👇
在快捷指令中运行 WLOC 恢复定位即可
